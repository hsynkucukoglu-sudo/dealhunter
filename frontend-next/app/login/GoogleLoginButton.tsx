...
export function GoogleLoginButton() {
  ...
  async function handleClick() {
    ...
    try {
      if (isCapacitor()) {
        // Chrome Custom Tabs açarak Google OAuth yap (WebView'da çalışmaz)
        const { Browser } = await import('@capacitor/browser')
        await Browser.open({
          url: 'https://www.dealhunter4u.nl/api/auth/signin/google',
          presentationStyle: 'popover',
        })
        // Kullanıcı browser'ı kapattığında sayfayı yenile (session cookie alınır)
